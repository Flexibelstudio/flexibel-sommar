...
interface FireIconProps extends IconProps {
  streakLevel?: number; // 0: no streak, 1: small, 2: medium, 3: large
}

export const FireIcon: React.FC<FireIconProps> = ({ className, streakLevel = 0 }) => {
  let iconSizeClass = 'w-10 h-10 sm:w-12 sm:h-12'; // Default for level 0 and 1
  let animationClass = '';
  let colorClass = 'text-gray-400'; // Default for level 0 (no streak)

  if (streakLevel === 1) { // Small streak (1-3 days)
    colorClass = 'text-orange-500';
    animationClass = 'animate-pulseSoftFire';
  } else if (streakLevel === 2) { // Medium streak (4-6 days)
    iconSizeClass = 'w-12 h-12 sm:w-14 sm:h-14';
    colorClass = 'text-red-500';
    animationClass = 'animate-pulseMediumFire';
  } else if (streakLevel >= 3) { // Large streak (7+ days)
