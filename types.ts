...
export interface Level {
  ...
  icon?: string; // Optional icon representation for the level
}
...
export interface DiplomaData {
  ...
  workoutDurationSeconds?: number; // Added to store workout duration
  totalWorkoutsCompleted: number;
  comment?: string;
  didLevelUp?: boolean;
  score?: string; // Current workout's score (AMRAP/TimeCap)
  isNewRecord?: boolean; // Was the score a new record?
  previousBestScore?: string; // Previous best score for this workout
}
...
export interface CurrentWorkoutCompletionData {
  ...
  completedTimeSeconds?: number; // Actual time taken for Time Cap workouts
}
...
export interface WalkingLogEntry {
  ...
  durationMinutes: number; // Should be actual duration, but >= WALKING_CHALLENGE_DAILY_MINUTES for completion
  levelNameAtCompletion: string;
  streakAtCompletion: number;
  distance?: string; // e.g., "3.5 km"
  steps?: string;    // e.g., "4500 steg"
  actualStartTime?: string; // ISO string for the actual start time of the walk session
  comment?: string; // New field for comments on walks
}

export interface CurrentWalkingCompletionData {
  dateTime: Date; // This is the end time
  ...
  didLevelUp: boolean;
  abortedByUser: boolean; // True if duration < 30 min or user explicitly aborted early
  completedDurationSeconds?: number; // Actual duration of the walk in seconds
  distance?: string;
  steps?: string;
  actualStartTime?: Date; // Actual start time of the walk session
  comment?: string; // New field for comments on walks
}
...
export interface WalkingDiplomaData {
  ...
  completionDateTime: Date; // This is effectively the End Time
  ...
  actualStartTimestamp?: number; // Timestamp of when the walk started
  comment?: string; // New field for comments on walks
}
...
export interface AchievementCheckData {
  ...
  currentWalkingLevel: Level | null; // Can be null if walking hasn't started
  totalWorkoutsCompleted: number;
  totalWalkingDaysCompleted: number;
  currentWorkoutStreak: number;
  currentWalkingStreak: number;
  appShareCount: number; // Added for share achievements
}
...
export interface AchievementDefinition {
  ...
  name: string;
  description: string;
  icon: React.FC<{ className?: string; solid?: boolean }>; // Changed to React.FC
...
export interface SummerStatusLevel {
  name: string;
  minScore: number;
  icon?: React.FC<{ className?: string; solid?: boolean }>; // Optional icon for the level title
}
