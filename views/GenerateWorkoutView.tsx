...
`;
    ...
    try {
      ...
      if (parsedData.format === WorkoutFormat.AMRAP || parsedData.format === WorkoutFormat.EMOM || parsedData.format === WorkoutFormat.TIME_CAP) {
        ...
        for (const segment of workSegments) {
          if (!segment.exercises || segment.exercises.length === 0) {
            ...
            throw new Error(APP_STRINGS.errorAIWorkoutMissingExercises);
          }
        }
      }
      
      const workoutFromAI: Omit<Workout, 'id'> = parsedData;

      const finalWorkout: Workout = {
        ...workoutFromAI,
        id: `generated-${Date.now()}`, // Keep prefix for achievement tracking
        type: params.type, 
        timedSegments: [ 
          PREPARE_SEGMENT, 
          ...(workoutFromAI.timedSegments || [])
        ],
      };
      ...
      if (err instanceof SyntaxError) {
          detailedErrorMessage = `Fel vid tolkning av AI-svar (JSON SyntaxError): ${err.message}. Försök igen, eller kontrollera bartenderns recept. Recept: ${jsonStr ? jsonStr.substring(0, 500) : "[Inget recept mottaget eller fel innan tolkning]"}`;
      ...
      } else if (err.message === APP_STRINGS.errorAIWorkoutMissingExercises) {
          detailedErrorMessage = APP_STRINGS.errorAIWorkoutMissingExercises;
      }
