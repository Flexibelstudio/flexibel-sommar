...
export const PostWalkingView = ({ onNavigate, userName, completionData }: PostWalkingViewProps): JSX.Element | null => {
  ...
  const [commentInput, setCommentInput] = useState(completionData.comment || ''); // New state for comment
  const [showWalkingLevelUpAnim, setShowWalkingLevelUpAnim] = useState(false); // State for level up animation
  ...
  const actualDurationSeconds = completionData.completedDurationSeconds ?? 0;
  const challengeDayConsideredComplete = actualDurationSeconds >= (WALKING_CHALLENGE_DAILY_MINUTES * 60);

  const formattedActualStartTime = formatTimeFromDate(completionData.actualStartTime);
  const formattedEndTime = formatTimeFromDate(completionData.dateTime);


  useEffect(() => {
    if (!challengeDayConsideredComplete) {
      let abortedMessage = APP_STRINGS.postWalkingAbortedInfo;
      if (actualDurationSeconds > 0) {
          abortedMessage = `Bra kämpat! Du gick i ${formatDurationFromSeconds(actualDurationSeconds)}. Kom ihåg att minst 30 minuter behövs för att klara dagens utmaning.`;
      }
    ...
    if (!completionData.abortedByUser && actualDurationSeconds >= WALKING_CHALLENGE_DAILY_MINUTES * 60) {
        triggerConfetti(completionData.didLevelUp); 
    }
    if (completionData.didLevelUp && !completionData.abortedByUser) {
        triggerHeartsAnimation();
        setShowWalkingLevelUpAnim(true); // Trigger level up animation
    }
    ...
    // Start/End times are now displayed separately. Only add duration to messages.
    newMessages.push(`Total promenadtid: ${formatDurationFromSeconds(actualDurationSeconds)}.`);
  ...
  }, [completionData, nameOrDefault, actualDurationSeconds, challengeDayConsideredComplete]);

  const passDataToNavigate = (targetView: View, diplomaDataForNavigation?: WalkingDiplomaData) => {
    const navigationPayload: any = {
        completedDurationSeconds: actualDurationSeconds, 
        initialTimestamp: completionData.actualStartTime?.getTime(), // Pass timestamp for log update
    };
    if (distanceInput.trim()) navigationPayload.distance = distanceInput.trim();
    if (stepsInput.trim()) navigationPayload.steps = stepsInput.trim();
    if (commentInput.trim()) navigationPayload.comment = commentInput.trim(); // Add comment to payload

    if (targetView === View.WalkingDiploma && diplomaDataForNavigation) {
        onNavigate(targetView, diplomaDataForNavigation);
    } else {
        onNavigate(targetView, navigationPayload);
    }
  };
  ...
  const handleShowWalkingDiploma = () => {
    ...
    const historicalLogs = walkingLogs.filter(
      log => !(log.challengeDay === completionData.newChallengeDay && log.dateCompleted === completionData.dateTime.toISOString().split('T')[0])
    );
    ...
    const diplomaDataToPass: WalkingDiplomaData = {
      ...
      completionDateTime: completionData.dateTime, // End time
      actualStartTimestamp: completionData.actualStartTime?.getTime(), // Start timestamp
      ...
      comment: commentInput.trim() || undefined, // Add comment to diploma data
    };
    passDataToNavigate(View.WalkingDiploma, diplomaDataToPass);
  };
    ...
    return (
      <div className="flex flex-col min-h-screen p-4 sm:p-6 bg-red-50 text-gray-800">
        <div className="mt-2 mb-6 self-start">
          <Button
            onClick={() => passDataToNavigate(View.Home)}
            variant="ghost"
  ...
  return (
    <div className="flex flex-col min-h-screen p-6 bg-sky-50 text-gray-800 relative"> {/* Added relative */}
      ...
      <div className="flex-grow flex flex-col items-center justify-center text-center">
        ...
        <h2 className="text-3xl sm:text-4xl font-bold text-sky-700 mb-2">
          {isChallengeFullyCompletedOver30Days ? APP_STRINGS.postWalkingChallengeCompletedMessage.replace('{name}', nameOrDefault) : APP_STRINGS.postWalkingTitle} 
        </h2>
        ...
        {/* Prominent Start and End Times */}
        {challengeDayConsideredComplete && completionData.actualStartTime && completionData.dateTime && (
          <div className="my-4 p-3 bg-sky-100 border border-sky-300 rounded-lg shadow-sm w-full max-w-md text-left">
            <p className="text-md text-sky-800">
              <span className="font-semibold">{APP_STRINGS.walkingDiplomaStartTimeLabel}</span>{' '}
              <span className="font-medium">{formattedActualStartTime}</span>
            </p>
            <p className="text-md text-sky-800 mt-1">
              <span className="font-semibold">{APP_STRINGS.walkingDiplomaEndTimeLabel}</span>{' '}
              ...
              <div className="my-6 w-full max-w-sm space-y-4">
                  ...
                  <div>
                      <label htmlFor="walkingComment" className="block text-sm font-medium text-gray-700 mb-1">
                        Kommentar (valfritt, t.ex. gick med Vän):
                      </label>
                      <textarea
                        id="walkingComment"
                        value={commentInput}
                        onChange={(e) => setCommentInput(e.target.value)}
                        placeholder="Hur kändes promenaden? Något speciellt?"
                        rows={3}
                        className="w-full p-2 border border-gray-300 rounded-md shadow-sm focus:ring-sky-500 focus:border-sky-500 bg-white text-gray-800 placeholder-gray-400"
                      />
                  </div>
        ...
        <div className="mt-6 space-y-4 w-full max-w-xs">
          ...
          {/* The "Back to Home" button is now at the top */}
        </div>
      </div>
    </div>
  );
