...
export const WorkoutDetailView: React.FC<WorkoutDetailViewProps> = ({ workout, onNavigate }) => {
  ...
  const isAiWorkout = workout.id.startsWith('generated-');
  const startButtonText = isAiWorkout ? APP_STRINGS.startGeneratedWorkoutButton : APP_STRINGS.startWorkout;
  // Consistent green styling for AI workout start button, now using thematic pink for "Låt festen börja!"
  const startButtonAdditionalClasses = isAiWorkout 
  ...
  return (
    <div className="flex flex-col min-h-screen p-4 sm:p-6 bg-white text-gray-800">
      ...
      <div className="flex-grow flex flex-col items-center">
        ...
        <Button 
          onClick={() => onNavigate(View.PreWorkoutCountdown, workout)}
          className={`w-full max-w-sm text-lg ${startButtonAdditionalClasses}`}
        >
          {isAiWorkout && <DrinkIcon className="w-6 h-6 mr-2" />}
          {startButtonText}
        </Button>
      </div>
    </div>
